...
bool QuadRenderer::isInitialized = false;
VertexShaderPtr QuadRenderer::sharedVS = nullptr;
PixelShaderPtr QuadRenderer::sharedPS = nullptr;
VertexBufferPtr QuadRenderer::sharedVB = nullptr;
IndexBufferPtr QuadRenderer::sharedIB = nullptr;

QuadRenderer::QuadRenderer() : Renderer() 
{
	this->setSize(9.0f);
	this->hasTexture = false;
	this->setName("Quad Renderer");
}
...
void QuadRenderer::setUpVerticesAndIndices(vertex* vertex_list, unsigned int* index_list)
{
	...
	{
		{ Vector3D(-0.5f * size,  -0.5f * size, 0.0f ),  Vector2D(0.0f, 1.0f) },  // 0
		{ Vector3D(-0.5f * size,   0.5f * size, 0.0f ),  Vector2D(0.0f, 0.0f) },  // 1
		{ Vector3D(0.5f  * size,   0.5f * size, 0.0f ),  Vector2D(1.0f, 0.0f) },  // 2
		{ Vector3D(0.5f  * size,  -0.5f * size, 0.0f ),  Vector2D(1.0f, 1.0f) }   // 3
	};
...
void QuadRenderer::update(constant cc, int width, int height, int camIndex)
{
...
{
	QuadRenderer::sharedVS.reset();
	QuadRenderer::sharedPS.reset();
	QuadRenderer::sharedVB.reset();
	QuadRenderer::sharedIB.reset();
}
